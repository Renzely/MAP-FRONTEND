...
import cosmeticlogo from "../../Images/Marabou_Logo/MARABOU - COSMETIQUE ASIA.jpg";
  ...
  const handleSaveChanges = async (updatedEmployee) => {
    ...
    if (updatedEmployee.sss && updatedEmployee.sss.length !== 10) {
      Swal.fire({
        ...
        text: "SSS must be exactly 10 digits.",
        icon: "error",
    ...
    if (
      ...
      Swal.fire({
        ...
        text: "PhilHealth must be exactly 12 digits.",
        icon: "error",
        confirmButtonColor: "#d32f2f",
      });
      return;
    ...
    if (updatedEmployee.hdmf && updatedEmployee.hdmf.length !== 12) {
      Swal.fire({
        title: "Invalid HDMF Number",
        text: "HDMF must be exactly 12 digits.",
        icon: "error",
        confirmButtonColor: "#d32f2f",
      });
      return;
    ...
    if (updatedEmployee.tin && updatedEmployee.tin.length !== 12) {
      Swal.fire({
        ...
        text: "TIN must be exactly 12 digits.",
        icon: "error",
        confirmButtonColor: "#d32f2f",
      });
      return;
      ...
      try {
        ...
        const bmpowerAccounts = response.data.filter(
          (acc) =>
            ...
            acc.clientAssigned?.toUpperCase() === "COSMETIQUE ASIA",
        );
    ...
    try {
      const response = await axios.post(
        "https://api-map.bmphrc.com/export-merch-accounts",
        {
          ...
          clientAssigned: "COSMETIQUE ASIA",
        },
      );
