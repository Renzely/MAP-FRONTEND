      ...
      try {
        ...
        const bmpowerAccounts = response.data.filter(
          (acc) =>
            ...
            acc.clientAssigned?.toUpperCase() === "UNION GALVASTEEL CO"
        );
    ...
    try {
      const response = await axios.post(
        "https://api-map.bmphrc.com/export-merch-accounts",
        {
          ...
          clientAssigned: "UNION GALVASTEEL CO",
        }
      );
      ...
      link.download = `MASTERLIST_Union-Galvasteel_${
      ...
      <Box sx={{ display: "flex", flexDirection: { xs: "column", md: "row" } }}>
        ...
        >
          <Typography variant="h5" sx={{ mb: 2 }}>
            Employee Accounts for UNION GALVASTEEL CO
          </Typography>
          <Box
            sx={{
              display: "flex",
