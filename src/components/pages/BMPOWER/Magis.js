...
export default function BmpowerHO() {
  ...
  const fetchSavedRequirements = async (employeeEmail) => {
    try {
      const response = await axios.get(
        "http://192.168.68.54:3001/get-merch-accounts",
      );
  ...
  const handleSaveChanges = async (updatedEmployee) => {
    try {
      ...
      await axios.put(
        `http://192.168.68.54:3001/update-employee/${updatedEmployee._id}`,
        payload,
      );
      ...
      try {
        ...
        const bmpowerAccounts = response.data.filter(
          (acc) =>
            ...
            acc.clientAssigned?.toUpperCase() === "MAGIS DISTRIBUTION INC.",
        );
  ...
  const getExportData = async () => {
    try {
      const response = await axios.post(
        "http://192.168.68.54:3001/export-merch-accounts",
        {
          remarks: selectedRemarks,
          clientAssigned: "MAGIS DISTRIBUTION INC.",
        },
      );
      ...
      link.download = `MASTERLIST_MAGIS_${
                                ...
                                onChange={async (e) => {
                                  ...
                                  try {
                                    ...
                                    for (const file of files) {
                                      const response = await axios.post(
                                        "http://192.168.68.54:3001/save-requirements-images",
                                        {
                                          fileName: file.name,
                                          fileType: file.type,
                                        },
