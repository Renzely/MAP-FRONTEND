      ...
      try {
        ...
        const bmpowerAccounts = response.data.filter(
          (acc) =>
            ...
            acc.clientAssigned?.toUpperCase() === "ROYAL CANIN PHILS."
        );
    ...
    try {
      const response = await axios.post(
        "https://api-map.bmphrc.com/export-merch-accounts",
        {
          ...
          clientAssigned: "ROYAL CANIN PHILS.",
        }
      );
      ...
      link.download = `MASTERLIST_Royal-Canin_${
      ...
      <Box sx={{ display: "flex", flexDirection: { xs: "column", md: "row" } }}>
        ...
        >
          <Typography variant="h5" sx={{ mb: 2 }}>
            Employee Accounts for ROYAL CANIN PHILS.
          </Typography>
          <Box
            sx={{
              display: "flex",
